//! Browser playtest shell for `jrpg-runner`: [`WasmRunner`].
//!
...
//! [`GbButton`](jrpg_renderer::input::GbButton):
//!
//! | bit | button |
//! |-----|--------|
//! | 0   | A      |
...
use jrpg_renderer::input::InputState;
use jrpg_runner::vfs::MemoryFiles;
use jrpg_runner::{LoadedProject, RunnerGame, RunnerOptions, SCREEN_H, SCREEN_W};
use wasm_bindgen::prelude::*;

/// Game Boy frame rate the runner is tuned for (matches jrpg-app's window
/// loop); audio PCM is generated at `SAMPLE_RATE / GB_FRAME_RATE` per tick.
const GB_FRAME_RATE: f32 = 59.7275;

/// Audio sample rate of the rendered PCM (matches jrpg-runner's `SAMPLE_RATE`).
...
/// (320×240, from `jrpg_runner`'s `game.rs` constants); `tick` returns it as
...
mod tests {
    ...
    fn boot_fails_with_named_step() {
        ...
        assert!(err.contains(".jrpg-editor.json"), "{err}");
    }
}
