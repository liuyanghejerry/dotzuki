//! Native integration test: boot the committed `jrpg-runner` fixture project
...
use jrpg_runner_web::WasmRunner;

/// Flat colours of the four generated tiles — the same recipe as
/// `jrpg-runner/tests/fixture.rs` (no binary assets committed).
...
/// jrpg-runner tests generate in code added for both maps.
...
fn fixture_files() -> HashMap<String, String> {
    let fixture =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../jrpg-runner/tests/fixtures/demo");
...
/// A minimal music track (jrpg-audio `TrackDef` JSON), same as
/// `jrpg-runner/tests/audio.rs`.
