...
impl TestDir {
    fn new(test: &str) -> Self {
        ...
        let dir = std::env::temp_dir().join(format!("jrpg-cli-{test}-{}-{id}", std::process::id()));
...
fn new_creates_expected_tree_and_manifest() {
    ...
    let manifest: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(project.join(".jrpg-editor.json")).unwrap(),
    )
    .unwrap();
...
fn new_honors_title_override() {
    ...
    let manifest: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(tmp.path().join("my-game/.jrpg-editor.json")).unwrap(),
    )
    .unwrap();
...
fn new_scaffolds_into_existing_empty_dir() {
    ...
    let project = scaffold(tmp.path(), "my-game");
    assert!(project.join(".jrpg-editor.json").is_file());
}
...
fn check_fails_without_manifest() {
    ...
    assert!(!out.status.success());
    assert!(stderr(&out).contains("No .jrpg-editor.json found"));
}
