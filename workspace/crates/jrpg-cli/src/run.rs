...
//! `jrpg_app`; `--headless` drives the game windowless for CI smoke tests,
...
//! save file (`<project>/.jrpg-save.json`) resumes on boot — `--fresh`
...
use jrpg_runner::{run_headless, HeadlessOptions, LoadedProject, RunnerGame, RunnerOptions};
...
pub fn run(args: RunArgs) -> Result<()> {
    ...
    jrpg_app::run(
        jrpg_app::GameWindowConfig {
            title,
            width: jrpg_runner::SCREEN_W as u32,
            height: jrpg_runner::SCREEN_H as u32,
            scale: args.scale,
            resizable: true,
        },
        game,
