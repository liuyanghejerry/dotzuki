...
//! present on disk) must parse as a jrpg-rules `Ruleset`. Record JSONs are
...
use jrpg_engine_dsl::compiler::compile_dirs;
use jrpg_runner::manifest::{Manifest, TableDef, DEFAULT_RULES_FILE};

pub fn run(dir: &Path) -> Result<()> {
    let manifest_path = dir.join(".jrpg-editor.json");
    if !manifest_path.is_file() {
        bail!(
            "No .jrpg-editor.json found in {} — not a jrpg game project",
            dir.display()
        );
    }
...
fn battle_diagnostics(manifest: &Manifest, root: &Path) -> Vec<String> {
    ...
    for (what, table) in [("party", &party), ("enemies", &enemies)] {
        ...
        let skills_field = battle
            ...
            .unwrap_or(jrpg_runner::manifest::DEFAULT_SKILLS_FIELD);
    ...
    if rules_path.is_file() {
        match fs::read_to_string(&rules_path) {
            Ok(text) => {
                for d in jrpg_runner::validate_ruleset(&text) {
    ...
    impl TestDir {
        fn new(test: &str) -> Self {
            ...
            let dir = std::env::temp_dir()
                .join(format!("jrpg-cli-check-{test}-{}-{id}", std::process::id()));
