...
//! (`tools/jrpg-editor/server/scaffold.ts`, "empty" template) plus the `game`
...
/// Activity list written into `.jrpg-editor.json` — the same six the editor
...
pub fn run(name: &str, parent: Option<&Path>, title: Option<&str>) -> Result<PathBuf> {
    ...
    fs::write(target.join(".jrpg-editor.json"), manifest_json)?;
    ...
    println!("  .jrpg-editor.json");
    println!("  data/maps/  data/tiles/  gfx/");
    println!("  assets/scenes/main.scene");
    println!("  README.md");
    println!("Next: open the folder in jrpg-editor, or run `jrpg check {}`.", target.display());
