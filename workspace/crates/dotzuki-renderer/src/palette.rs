...
use dotzuki_engine::palette::{
    PaletteProvider, SgbColor, SgbPaletteEntry, SgbPaletteId, SetPalCommand,
};
...
pub struct ColorPaletteState<'a> {
    ...
    pub palette_provider: &'a dyn PaletteProvider<SgbPaletteId>,
...
impl<'a> ColorPaletteState<'a> {
    ...
    pub fn set_battle_palette(
        ...
        let player_pal = if player_transformed {
            SgbPaletteId::GrayMon
        } else {
            self.palette_provider.monster_palette(player_species)
        };
        let enemy_pal = if enemy_transformed {
            SgbPaletteId::GrayMon
        } else {
            self.palette_provider.monster_palette(enemy_species)
        };
