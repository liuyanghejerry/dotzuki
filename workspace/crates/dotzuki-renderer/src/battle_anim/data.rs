...
/// are pixel bytes, matching the `dbsprite` macro expansion
/// (macros/gfx.asm): Y = y_tile*8 + y_px, X = x_tile*8 + x_px.
...
// From data/battle_anims/special_effects.asm (AnimationIdSpecialEffects).
// PlaySubanimation runs the hook after each drawn frame block, keyed by
// wAnimationID (1-based animation id, i.e. MOVE_ANIM_DATA index + 1).

/// (animation id, hook) pairs — the 24 entries of AnimationIdSpecialEffects,
/// in the original table's order.
pub static ANIMATION_ID_HOOKS: [(u8, FrameHook); 24] = [
    ...
    (0xAA, FrameHook::TradeHidePokemon),      // TRADE_BALL_DROP_ANIM
    (0xAB, FrameHook::TradeShakePokeball),    // TRADE_BALL_SHAKE_ANIM
    (0xAC, FrameHook::TradeJumpPokeball),     // TRADE_BALL_TILT_ANIM
...
mod tests {
    ...
    // === BASE_COORDS spot checks (from data/battle_anims/base_coords.asm) ===
    ...
    // === FRAME_BLOCK_DATA spot checks (from data/battle_anims/frame_blocks.asm) ===
    ...
    // === SUBANIM_DATA spot checks (from data/battle_anims/subanimations.asm) ===
    ...
    // === MOVE_ANIM_DATA spot checks (from data/moves/animations.asm) ===
