...
//!
//! Models the audio engine (audio/engine_1.asm). The engine has
...
impl Sequencer {
    ...
    ///
    /// Mirrors `Audio2_OverwriteChannelPointer` (audio/poke_flute.asm:13-18),
    /// which pokes a new pointer straight into `wChannelCommandPointers` while
    /// the channel is live. `Music_PokeFluteInBattle` uses this to hijack the
    /// SFX channels right after `PlaySound` has initialized them, so the new
    /// stream starts executing on the next frame.
    ///
    ...
    pub fn update_frame(&mut self, apu: &mut Apu) {
        ...
        // audio/engine_1.asm:547-552 (Audio1_volume) writes the param byte
        // straight to rAUDVOL (NR50) in the middle of command processing,
        // so it takes effect on the same frame the command executes.
    ...
    fn execute_commands(&mut self, ch_idx: usize) {
        ...
        loop {
            ...
            match cmd {
                ...
                Command::Volume(vol) => {
                    // audio/engine_1.asm:547-552 (Audio1_volume): the param
                    // byte is written directly to rAUDVOL (NR50), effective
                    // immediately mid-song (not deferred to the next note).
                    // The write lands at the end of this update_frame.
    ...
    ///
    /// audio/engine_1.asm:673-688 (`.drum_note`/`.playDnote`): the instrument
    /// byte is a SFX ID (SFX_NOISE_INSTRUMENT01..19 = IDs 1-19, see
    /// audio/headers/sfxheaders1.asm) played through Audio1_PlaySound. The
    /// music noise channel (CHAN4) itself only advances its note delay —
    /// Audio1_note_length returns without any hardware write because CHAN4
    /// has BIT_NOISE_OR_SFX set (engine_1.asm:743-746). All drum hardware
    /// output comes from the one-shot SFX running on CHAN8.
    ///
    /// The delay is computed from the length nibble with the channel's
    /// note_speed and the music tempo (CHAN4 is a music channel,
    /// engine_1.asm:690-710) — unchanged from before.
    ...
    ///
    /// Priority rule from Audio1_PlaySound (engine_1.asm:1455-1462): if CHAN8
    ...
    /// IDs >= NOISE_INSTRUMENTS_END *do* override a playing drum,
    /// engine_1.asm:1463-1467; those go through the normal `play_sfx` path.)
    ///
    /// The original also gates on wDisableChannelOutputWhenSfxEnds
    /// (engine_1.asm:681-683), but that flag is only ever set by the unused
    /// unknownmusic0xef command ($EF, "appears to never be used",
    /// engine_1.asm:513-530), so it is always clear in practice.
    fn trigger_drum_instrument(&mut self, instrument: u8) {
        ...
        if self.channels[CHAN8].active {
            // CHAN8 busy: drum hit dropped (engine_1.asm:1460-1462).
            return;
        }
    ...
    fn handle_sfx_noise_note(
        ...
        // wSfxTempo (engine_1.asm:711-715, Audio1_note_length .sfxChannel:
        // `ld d, $1 / ld e, $0 / cp CHAN8 / jr z, .skip`).
        let tempo = 0x0100;
        let (delay, new_frac) =
            commands::calculate_delay(length + 1, ch.note_speed, tempo, ch.delay_frac);
        ch.delay_counter = delay;
