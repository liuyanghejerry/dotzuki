...
//! format and converts them into [`jrpg_engine`] types suitable for rendering
//! and game logic.
//!
//! # GID flipping
//!
...
//!
//! ```rust
//! use jrpg_engine_tiled::{parse_tmx, tmx_to_map_state};
//!
//! let json = r#"{
...
use jrpg_engine::render::{BlendMode, MapLayer, MapRenderState};
...
///
/// ```rust
/// use jrpg_engine_tiled::PropertyConfig;
///
...
/// ```rust
/// use std::collections::HashMap;
/// use jrpg_engine_tiled::{TileProperties, parse_tile_properties};
///
...
/// The caller must provide a `map_id` string (e.g. `"PalletTown"`) that will be
...
///
/// ```rust
/// use jrpg_engine_tiled::gid_to_tilemap_entry;
///
...
// ---------------------------------------------------------------------------
// Conversion to jrpg-engine types
// ---------------------------------------------------------------------------
...
mod tests {
  ...
  "tilesets": [
    ...
    fn trigger_script_name() {
        let props = make_props(&[("trigger", "oak_lab_door")]);
        let tp = parse_tile_properties(&props);
        assert_eq!(tp.trigger.as_deref(), Some("oak_lab_door"));
    }
