//! Integration tests for `jrpg-runner` against the committed fixture project
...
use jrpg_runner::map::{MapObjects, RuntimeMap};
use jrpg_runner::tileset::PngTileset;
use jrpg_runner::LoadedProject;
...
fn entry_falls_back_when_game_section_absent() {
    ...
    let manifest: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(root.join(".jrpg-editor.json")).unwrap(),
    )
    .unwrap();
...
fn load_fails_without_manifest() {
    ...
    assert!(format!("{err:#}").contains(".jrpg-editor.json"));
}
