...
use jrpg_runner::battle::BattleSetup;
use jrpg_runner::{LoadedProject, RunnerGame, RunnerOptions};
...
fn setup_loads_records_and_chart() {
    ...
    let rng: Box<dyn jrpg_runner::battle::BattleRng> =
        Box::new(jrpg_runner::battle::ScriptedRng::new(vec![50, 100, 1]));
...
fn scene_battle_win_path_resumes_scene_and_harvests_flags() {
    ...
    let mut fb = jrpg_engine::render::FrameBuffer::new(
        jrpg_engine::render_config::RenderConfig::new(320, 240),
        jrpg_engine::render::Rgba::BLACK,
    );
    game.draw(&mut fb);
...
fn scene_battle_without_battle_section_autowins() {
    ...
    let mut manifest: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(root.join(".jrpg-editor.json")).unwrap()).unwrap();
    manifest.as_object_mut().unwrap().remove("battle");
    fs::write(
        root.join(".jrpg-editor.json"),
...
fn ron_record_overrides_table_power_element_and_cost() {
    ...
    assert_eq!(battle.outcome(), Some(jrpg_runner::BattleOutcome::Win));
    let log = battle.log();
...
fn write_items(root: &Path) {
    let path = root.join(".jrpg-editor.json");
    let mut manifest: serde_json::Value =
...
fn party_state_and_inventory_round_trip_through_the_save() {
    ...
    assert_eq!(raw["version"], jrpg_runner::SAVE_VERSION);
...
fn encounter_resolution_order_and_validation() {
    ...
    let rng = || {
        Box::new(jrpg_runner::battle::ScriptedRng::new(vec![50, 100, 1]))
            as Box<dyn jrpg_runner::battle::BattleRng>
    };
