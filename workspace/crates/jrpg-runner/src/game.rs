...
//! pattern), textbox/choice UI from `jrpg-ui`, and placeholder sprites drawn
...
use jrpg_engine::overworld::actor::{frame_col, OverworldActor, OverworldCollision};
...
use jrpg_renderer::walk_sprite::WalkSprite;
use jrpg_ui::widgets::dialog::{draw_dialog, wrap_lines};
...
/// `jrpg_renderer::embedded_font::char_advance`).
...
/// or via the `jrpg_app::GameLoop` impl (windowed).
pub struct RunnerGame {
    ...
    audio: RunnerAudio,
    /// Where the save file lives (`<project>/.jrpg-save.json` by default).
    save_path: PathBuf,
...
impl RunnerGame {
    ...
    pub fn new(project: LoadedProject, opts: RunnerOptions) -> Result<Self> {
        ...
        if map_ids.is_empty() {
            ...
            log::info!("jrpg-runner: no maps; booting dialogue-only scene '{scene}'");
...
#[cfg(not(target_arch = "wasm32"))]
impl jrpg_app::GameLoop for RunnerGame {
