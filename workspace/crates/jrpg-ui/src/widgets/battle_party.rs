...
mod tests {
    ...
    fn test_config() -> MenuConfig {
        MenuConfig::new(TileRect::new(1,3,18,8), None, TileRect::new(2,4,16,6), jrpg_engine::menu::CursorStyle::new(Some(223), Default::default()))
    }
    fn test_data() -> BattlePartyData {
        BattlePartyData {
            entries: vec![
                BattlePartyEntry { name: "PIKACHU".into(), hp: 20, max_hp: 35, status: Some("BRN".into()), active: true },
                BattlePartyEntry { name: "BULBASAUR".into(), hp: 45, max_hp: 45, status: None, active: true },
                BattlePartyEntry { name: "CHARMANDER".into(), hp: 0, max_hp: 39, status: Some("FNT".into()), active: false },
            ],
            cursor: 0,
        }
    }
    ...
    #[test] fn draws_names() {
        ...
        assert!(painter.texts.iter().any(|(_,t,_)| t.contains("BULBASAUR")));
