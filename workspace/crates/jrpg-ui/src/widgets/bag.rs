...
mod tests {
    ...
    fn header_config() -> MenuConfig {
        MenuConfig::new(TileRect::new(0,0,8,3), None, TileRect::new(1,1,6,1), jrpg_engine::menu::CursorStyle::new(None, Default::default()))
    }
    fn list_config() -> MenuConfig {
        MenuConfig::new(TileRect::new(0,2,9,2), None, TileRect::new(1,3,7,0), jrpg_engine::menu::CursorStyle::new(Some(223), Default::default()))
    }
    fn test_data() -> BagData {
        BagData {
            items: vec![
                BagItemEntry { name: "POTION".into(), quantity: 5, index: 0 },
                BagItemEntry { name: "POKE BALL".into(), quantity: 3, index: 1 },
            ],
            cursor: 0,
        }
    }
    ...
    #[test] fn draws_items() {
        ...
        assert!(painter.texts.iter().any(|(_,t,_)| t.contains("POKE BALL")));
    }
