...
pub fn draw_oak_speech_phase<P: Painter>(text: &str, phase: usize, configs: &[MenuConfig], painter: &mut P) {
...
mod tests {
    ...
    fn test_config() -> MenuConfig {
        MenuConfig::new(TileRect::new(0,0,20,10), None, TileRect::new(1,1,18,8), jrpg_engine::menu::CursorStyle::new(Some(223), Default::default()))
    }

    #[test] fn draws_phase() {
        let mut painter = RecordingPainter::default();
        draw_oak_speech_phase("Hello!", 0, &[test_config()], &mut painter);
        assert!(painter.texts.iter().any(|(_,t,_)| t == "[1]"));
        assert!(painter.texts.iter().any(|(_,t,_)| t == "Hello!"));
    }
    #[test] fn draws_arrow() {
        let mut painter = RecordingPainter::default();
        draw_oak_speech_phase("Hi", 1, &[test_config()], &mut painter);
