...
pub fn draw_list_menu<P: Painter>(data: &ListMenuData, configs: &[MenuConfig], painter: &mut P) {
    ...
    let rect = jrpg_engine::render::TileRect::new(config.area.tx, config.area.ty, config.area.tw, eff_h);
...
mod tests {
    ...
    fn test_config() -> MenuConfig {
        MenuConfig::new(TileRect::new(5,3,10,1), None, TileRect::new(6,4,8,0), jrpg_engine::menu::CursorStyle::new(Some(223), Default::default()))
    }
