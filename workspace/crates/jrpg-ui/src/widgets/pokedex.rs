//! Pokédex entry widget — full-screen data card with custom border tiles,
//! divider line, stats, and paginated description text.
//! Uses absolute tile positions matching the original Game Boy Pokedex screen layout.

use jrpg_engine::menu::MenuConfig;
use jrpg_engine::render::{Rgba, Painter, TilePos, Ui};
...
/// GB Pokedex border tile IDs ($63–$6F)
mod border_tiles {
...
pub fn draw_pokedex<P: Painter>(entry: &PokedexEntry, _configs: &[MenuConfig], painter: &mut P) {
...
/// Draw the full-screen Pokedex border using GB tile indices.
...
mod tests {
    ...
    fn draws_full_screen_border() {
        ...
        draw_pokedex(&test_entry(), &[], &mut painter);
    ...
    fn draws_name() {
        let mut painter = RecordingPainter::default();
        draw_pokedex(&test_entry(), &[], &mut painter);
        assert!(painter.texts.iter().any(|(p, t, _)| p.tx == 9 && p.ty == 2 && t == "PIKACHU"));
    }
    ...
    fn draws_description_page_1() {
        ...
        entry.page = 1;
        let mut painter = RecordingPainter::default();
        draw_pokedex(&entry, &[], &mut painter);
