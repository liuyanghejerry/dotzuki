...
pub struct Trigger {
    /// Unique identifier for this trigger (e.g. `"pallet_town_oak"`).
    pub id: String,
    /// The map this trigger lives on (e.g. `"PalletTown"`).
    pub map_id: String,
...
///     "heal_npc", "PalletTown", TriggerType::OnStep,
///     3, 5, "healParty", true,
/// ));
///
/// // Every frame
/// let triggered: Vec<String> = mgr.check_triggers("PalletTown", 3, 5);
/// for name in &triggered {
///     script_engine.call_function(name);
/// }
///
/// // On A button press
/// if let Some(name) = mgr.check_interact_mut("PalletTown", facing_x, facing_y) {
///     script_engine.call_function(&name);
/// }
/// ```
