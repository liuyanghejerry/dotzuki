...
///
/// The engine ships the dispatch machinery with **zero Pokémon types**; all
...
pub trait EffectProvider: BattleProvider + 'static {
    ...
    /// Pokémon semantics. The `/16`, the toxic counter, and the ASM "status then
    ...
    /// Pokémon volatile — it only swaps one `BattleAction` for another) and
