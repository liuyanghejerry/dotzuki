//! # Hello JRPG — a minimal demo proving `jrpg-engine` is game-agnostic.
//!
//! Demonstrates: overworld movement, NPC interaction, turn-based battle,
//! dialog, save/load, menu navigation. Uses **only** `jrpg_engine` traits.
...
use jrpg_engine::map::MapTrait;
use jrpg_engine::menu::{MenuInput, MenuLayout, MenuOption, MenuProvider, MenuSystem};
...
use jrpg_engine::save::{InMemoryStorage, SaveData, SaveError, SaveManager, SaveSlot};
use jrpg_engine::text::{
...
impl ItemProvider for HelloConfig {
    ...
    fn item_kind(&self, item: &ItemKind) -> jrpg_engine::items::ItemKind<()> {
        let _ = item;
        jrpg_engine::items::ItemKind::Consumable
    }
}
...
fn demo_overworld() {
    ...
    let npc_positions = jrpg_engine::overworld::get_npc_positions(&npcs);
    ...
    jrpg_engine::overworld::advance_step(&mut state);
    jrpg_engine::overworld::advance_step(&mut state);
    // ...walking finishes after WALK_COUNTER_INIT (8) frames
    for _ in 0..8 { jrpg_engine::overworld::advance_step(&mut state); }
    println!("║ After walk: player at ({}, {})", state.player.x, state.player.y);

    // NPC interaction: check if NPC is nearby
    let interaction = jrpg_engine::overworld::try_interact(
...
mod tests {
    ...
    use jrpg_engine::menu::MenuAction;
