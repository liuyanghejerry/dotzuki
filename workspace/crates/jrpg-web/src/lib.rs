...
use jrpg_renderer::layout_engine::types::{DataContext, DataValue, RenderContext, Theme};
...
fn log_warn(msg: &str) {
    eprintln!("[jrpg-web] WARN: {}", msg);
}

/// Log an error message.
fn log_error(msg: &str) {
    eprintln!("[jrpg-web] ERROR: {}", msg);
}
...
/// WASM built on `jrpg-renderer`'s layout engine.
///
