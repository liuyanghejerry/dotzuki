...
game_scene PalletTown {
  ...
  @storyline("coordNorthExit") {
    @trigger(map = "PalletTown", coords = [[10, 1], [11, 1]], name = "northExit1")
    @speaker("") { "wait" }
  }
}
"#;
        let json_text = compile_scene_to_config(src, "PalletTown/script.scene").unwrap();
        let got: Value = serde_json::from_str(&json_text).unwrap();

        let expected = json!({
            "$schema": "../../schemas/script_config.schema.json",
            "onLoad": "PalletTownOnLoad",
            "npcs": [
                {"id": 1, "talk": "talkOak", "toggleId": "PALLET_TOWN_OBJ_1", "scriptId": "PALLETTOWN_OAK", "defaultHidden": true},
                {"id": 2, "talk": "talkGirl"}
            ],
