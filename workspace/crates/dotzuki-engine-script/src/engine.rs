...
impl ScriptEngine {
    ...
    /// registered. The `registrar` adds game-specific APIs such as `givePokemon`,
    /// `startBattle`, etc.
...
fn register_core_game_api(context: &mut Context, bridge: Rc<RefCell<SharedBridge>>) {
    ...
    // a line from a set each interaction (e.g. Cerulean gossip NPCs, SS Anne
    ...
    // S.S. Anne departure cutscene (VermilionDock): the ship-sail
    // animation runs as a blocking cutscene; the script resumes when it
    // completes.
    register_async_command!(
        "playShipDeparture",
        bridge,
        ...
        |_args: &[JsValue], _ctx: &mut Context| -> JsResult<ScriptCommand> {
            Ok(ScriptCommand::PlayShipDeparture)
        }
    );
    ...
    register_async_command!(
        ...
        game_obj,
        |_args: &[JsValue], _ctx: &mut Context| -> JsResult<ScriptCommand> {
            Ok(ScriptCommand::AnimateHealingMachine)
        }
    );
    ...
    register_async_command!(
        ...
        |args: &[JsValue], ctx: &mut Context| -> JsResult<ScriptCommand> {
            let species = args
                ...
                .to_string(ctx)?
                .to_std_string_lossy();
            Ok(ScriptCommand::OpenNamingScreen { species })
        }
    );

    // game.choosePartyPokemon() -> Promise<number>  (party index, or -1 if cancelled)
    register_async_command!(
        "choosePartyPokemon",
        bridge,
        context,
        game_obj,
        |_args: &[JsValue], _ctx: &mut Context| -> JsResult<ScriptCommand> {
            Ok(ScriptCommand::ChoosePartyPokemon)
        }
    );
    ...
    register_async_command!(
        ...
        |args: &[JsValue], ctx: &mut Context| -> JsResult<ScriptCommand> {
            ...
            let nickname = args
                ...
                .to_string(ctx)?
                .to_std_string_lossy();
            Ok(ScriptCommand::SetPartyNickname { index, nickname })
        }
    );
