...
//!     │
//!     ▼
//! CutsceneManager::start_cutscene("oak_lab_intro")
//!     │  sets active=true, current_script=Some("oak_lab_intro")
//!     │
//!     ▼
//! game loop: sees cutscene active → calls script_engine.call_function("oak_lab_intro")
//!     │
//!     ▼
//! JS: export async function oak_lab_intro() {
...
mod tests {
    ...
    fn test_start_and_end_cutscene() {
        ...
        cm.start_cutscene("oak_lab_intro", true);
        assert!(cm.is_active());
        assert!(cm.is_blocking());
        assert_eq!(cm.current_script_name(), Some("oak_lab_intro"));
    ...
    fn test_queue_script() {
        ...
        let next = cm.end_cutscene();
        assert_eq!(next, Some("oak_arrives".to_string()));
    }
