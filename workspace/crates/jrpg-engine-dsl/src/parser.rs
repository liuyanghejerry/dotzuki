...
mod tests {
    ...
    fn test_parse_speaker() {
        let tokens = vec![
            ...
            tok(Token::DirectiveSpeaker), tok(Token::LParen), tok(s_("Oak")), tok(Token::RParen), tok(Token::LBrace),
    ...
    fn test_parse_say_cutscene_line() {
        // `@say("Oak") { "text" }` parses as StoryStmt::Say (cutscene speech),
        // distinct from @speaker (player-initiated talk).
        let tokens = vec![
            tok(Token::KeywordGameScene), tok(id("Dialog")), tok(Token::LBrace),
            tok(Token::DirectiveStorylines), tok(Token::LBrace),
            tok(Token::DirectiveSay), tok(Token::LParen), tok(s_("Oak")), tok(Token::RParen), tok(Token::LBrace),
            tok(s_("Hello!")), tok(s_("Welcome!")), tok(Token::RBrace),
            tok(Token::RBrace), tok(Token::RBrace), tok(Token::Eof),
        ];
        let (doc, errors) = parse(tokens);
    ...
    fn test_parse_speaker_rejects_second_argument() {
        ...
        let tokens = vec![
            tok(Token::KeywordGameScene), tok(id("Dialog")), tok(Token::LBrace),
            tok(Token::DirectiveStorylines), tok(Token::LBrace),
            tok(Token::DirectiveSpeaker), tok(Token::LParen), tok(s_("Oak")), tok(Token::Comma), tok(id("auto")), tok(Token::RParen), tok(Token::LBrace),
    ...
    fn test_parse_full_scene() {
        let tokens = vec![
            ...
            tok(Token::DirectiveStorylines), tok(Token::LBrace),
            tok(Token::DirectiveSpeaker), tok(Token::LParen), tok(s_("Oak")), tok(Token::RParen), tok(Token::LBrace), tok(s_("Hello!")), tok(Token::RBrace),
            tok(Token::DirectiveChoice), tok(Token::LBrace),
    ...
    fn test_parse_directive_command_with_args() {
        let tokens = vec![
            tok(Token::KeywordGameScene), tok(id("Cmd")), tok(Token::LBrace),
            tok(Token::DirectiveStorylines), tok(Token::LBrace),
            tok(Token::DirectiveCommand), tok(Token::LParen), tok(s_("givePokemon")), tok(Token::Comma), tok(s_("PIKACHU")), tok(Token::Comma), tok(n(5.0)), tok(Token::RParen),
            tok(Token::Newline),
            tok(Token::RBrace), tok(Token::RBrace), tok(Token::Eof),
        ];
        let (doc, errors) = parse(tokens);
        ...
        match &sb.statements[0] {
            StoryStmt::Command { name, args, .. } => {
                assert_eq!(name, "givePokemon");
                assert_eq!(args.len(), 2);
                assert!(matches!(&args[0], Expression::StringLit(s) if s == "PIKACHU"));
    ...
    fn test_parse_named_storyline() {
        let tokens = vec![
            ...
            tok(Token::LBrace),
            tok(Token::DirectiveSpeaker), tok(Token::LParen), tok(s_("Oak")), tok(Token::RParen),
    ...
    fn test_parse_named_storyline_with_trigger() {
        let tokens = vec![
            ...
            tok(id("map")), tok(Token::Equals), tok(s_("OaksLab")), tok(Token::Comma),
            tok(id("npc")), tok(Token::Equals), tok(s_("Oak")),
            tok(Token::RParen),
            tok(Token::DirectiveSpeaker), tok(Token::LParen), tok(s_("Oak")), tok(Token::RParen),
            tok(Token::LBrace), tok(s_("Hi")), tok(Token::RBrace),
            tok(Token::RBrace),
        ...
        assert_eq!(tr.map, "OaksLab");
        assert_eq!(tr.npc.as_deref(), Some("Oak"));
    }
    ...
    fn test_parse_named_storyline_with_after() {
        let tokens = vec![
            ...
            tok(id("npc")), tok(Token::Equals), tok(s_("Oak")), tok(Token::Comma),
