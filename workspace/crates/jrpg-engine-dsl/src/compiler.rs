...
pub fn find_search_dirs(manifest_path: &Path) -> Vec<PathBuf> {
    ...
    // The `examples/*/assets` and `jrpg-template/assets` entries below are
...
fn compile_dsl_file_opt_with_components(
    ...
    match (&doc, effective_ext.as_str()) {
        (ast::Document::Scene(scene), "scene") => {
            ...
            js.push_str("// @generated by jrpg-engine-dsl — do not edit.\n");
            ...
            if let Some(ref ui) = scene.ui {
                ...
                if let Some(out_dir) = out_dir {
                    ...
                    let ui_with_header = format!(
                        "// @generated by jrpg-engine-dsl — do not edit.\n// Source: {}\n\n{}",
                        file_path, ui_json
                    );
...
pub fn compile_scene_to_js(source: &str, file_path: &str) -> Result<String, String> {
    ...
    match doc {
        ast::Document::Scene(scene) => {
            ...
            let mut js = String::new();
            js.push_str("// @generated by jrpg-engine-dsl\n");
            js.push_str(&format!("// Source: {}\n\n", file_path));
...
game_scene PalletTown {
  ...
  @storyline(\"talkOak\") {
    ...
    @speaker(\"Oak\") { \"Hello!\" }
        ...
        let result = compile_dsl_file("scene", content, "pallet.scene", &out_dir);
        assert!(result.is_ok(), "Compilation should succeed: {:?}", result.err());
        if let Ok(CompileOutput::Scene { js, .. }) = result {
            assert!(js.contains("export async function PalletTownOnLoad()"),
                "JS should contain PalletTownOnLoad, got:\n{}", js);
            assert!(js.contains("export async function storyline_talkOak()"),
                "JS should contain storyline_talkOak, got:\n{}", js);
...
game_scene OaksLab {
  @storyline("oak_ask") {
    @trigger(map = "OaksLab", npc = "Oak")
    @speaker("Oak") { "Hello!" }
  }
}
"#;
        let result = compile_dsl_file("scene", content, "oaks_lab.scene", &out_dir);
        assert!(result.is_ok(), "Compilation should succeed: {:?}", result.err());
        if let Ok(CompileOutput::Scene { routes, .. }) = result {
            assert_eq!(routes.len(), 1);
            assert_eq!(routes[0].map, "OaksLab");
            assert_eq!(routes[0].npc, Some("Oak".to_string()));
            assert!(!routes[0].on_enter);
            assert_eq!(routes[0].storyline, "oak_ask");
...
game_scene MartScene {
  @storyline("mart_pickup") {
    @trigger(map = "ViridianMart", on_enter = true)
...
"#;
        ...
        if let Ok(CompileOutput::Scene { routes, .. }) = result {
            assert_eq!(routes.len(), 1);
            assert_eq!(routes[0].map, "ViridianMart");
...
game_scene RivalScene {
  @storyline("rival_fight") {
    @trigger(map = "Route22", npc = "Blue", after = "oak_ask")
    @speaker("Blue") { "Let's battle!" }
  }
}
"#;
        ...
        if let Ok(CompileOutput::Scene { routes, .. }) = result {
            assert_eq!(routes.len(), 1);
            assert_eq!(routes[0].after, Some("oak_ask".to_string()));
        }
        cleanup(&out_dir);
    }
...
game_scene MultiScene {
  ...
  @storyline("nurse_heal") {
    @trigger(map = "PokeCenter", npc = "Nurse Joy")
    @speaker("Nurse Joy") { "Welcome!" }
  }
  @storyline("main") {
...
"#;
        ...
        if let Ok(CompileOutput::Scene { routes, .. }) = result {
            ...
            assert!(storylines.contains(&"oak_ask"));
    ...
    fn test_generate_routing_table_writes_json() {
        ...
        let routes = vec![
            RouteEntry {
                ...
                npc: Some("Oak".into()),
                on_enter: false,
                storyline: "oak_ask".into(),
                after: None,
            },
            RouteEntry {
                map: "ViridianMart".into(),
                npc: None,
                on_enter: true,
                storyline: "mart_pickup".into(),
                after: Some("oak_ask".into()),
            },
        ];
        ...
        assert_eq!(arr[0]["map"], "OaksLab");
        assert_eq!(arr[0]["npc"], "Oak");
        assert_eq!(arr[0]["storyline"], "oak_ask");
        assert!(arr[0]["after"].is_null());

        assert_eq!(arr[1]["map"], "ViridianMart");
        assert!(arr[1]["npc"].is_null());
        assert_eq!(arr[1]["onEnter"], true);
        assert_eq!(arr[1]["after"], "oak_ask");
...
game_scene Intro {
  @storyline(\"oak_ask\") {
    ...
    @speaker(\"Oak\") { \"Hello!\" }
        ...
        assert_eq!(report.routes[0].storyline, "oak_ask");
        assert!(
            report.scenes[0].1.contains("storyline_oak_ask"),
            "Scene JS should contain storyline_oak_ask, got:\n{}",
            report.scenes[0].1
        );
        assert!(
...
game_scene Conflict {
  ...
  @storyline(\"rival_challenge\") {
    ...
    @speaker(\"Oak\") { \"Fight!\" }
        ...
        assert!(report.diagnostics[0].contains("oak_ask"));
