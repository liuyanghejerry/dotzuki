...
//!
//! Run with: `cargo test -p jrpg-engine-dsl -- codegen_tests`

use jrpg_engine_dsl::ast::Document;
use jrpg_engine_dsl::codegen;
use jrpg_engine_dsl::lexer::Lexer;
use jrpg_engine_dsl::parser::Parser;
...
fn expect_scene(doc: &Document) -> &jrpg_engine_dsl::ast::GameScene {
    match doc {
...
fn compile_storylines(doc: &Document) -> (String, SourceMapBuilder) {
    ...
    let sb = jrpg_engine_dsl::ast::StorylineBlock {
...
game_scene Test {
  @storylines {
    ...
    assert!(js.contains("Oak:"), "Expected speaker prefix");
...
game_scene Test {
  @storylines {
    ...
    assert!(js.contains("Oak: Great!"));
    assert!(js.contains("Oak: Too bad."));
}
...
game_scene CmdTest {
  @storylines {
    ...
    assert!(
        js.contains("await game[\"givePokemon\"](\"PIKACHU\", 5)"),
        "should emit game[\"givePokemon\"]"
    );
    assert!(
        combined.contains("sourceMappingURL"),
...
game_scene Test {
  @storyline(\"delivery\") {
    ...
    assert!(
        js.contains("Oak: Package arrived!"),
...
game_scene Test {
  @storyline(\"checkStarter\") {
    @if (getFlag(\"GOT_STARTER\")) {
      @speaker(\"Oak\") { \"Already got one.\" }
    } @else {
      @speaker(\"Oak\") { \"Pick one.\" }
    }
  }
}";
    let doc = parse_scene(dsl);
