...
use jrpg_engine::render::{Rgba, TilePos, TileRect};

use jrpg_renderer::layout_engine::deserialize::parse_layout;
use jrpg_renderer::layout_engine::registry::ElementRegistry;
use jrpg_renderer::layout_engine::renderer::render_layout;
use jrpg_renderer::layout_engine::types::{DataContext, DataValue, RenderContext, ScreenLayout};
...
fn compile_gui(src: &str, path: &str) -> ScreenLayout {
    ...
    let (doc, errs) = jrpg_engine_dsl::parser::Parser::new(tokens, src).parse();
...
fn image_element_compiles_and_blits_registered_image() {
    use jrpg_renderer::layout_engine::types::{ImageData, ImageRegistry};
