...
/// Per-frame-block hook kinds from `data/battle_anims/special_effects.asm`
/// (`AnimationIdSpecialEffects`). In the original, `PlaySubanimation` calls
/// `DoSpecialEffectByAnimationId` after every `DrawFrameBlock`; the hook is
/// selected by `wAnimationID` and inspects `wSubAnimCounter` (which counts
/// down from the subanimation's frame count to 1).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FrameHook {
    /// `AnimationFlashScreen`: flash the screen after every frame block.
    FlashScreen,
    /// `FlashScreenEveryFourFrameBlocks` (Hyper Beam): flash when
    /// counter % 4 == 0.
    FlashScreenEveryFour,
    /// `FlashScreenEveryEightFrameBlocks` (Thunderbolt): flash when
    /// counter % 8 == 0.
    FlashScreenEveryEight,
    /// `DoBlizzardSpecialEffects`: flash at counters 13, 9, 5 and 1.
    BlizzardFlash,
    /// `DoExplodeSpecialEffects` (Selfdestruct/Explosion): flash every 4
    /// frame blocks; hide the attacking mon at counter 1.
    Explode,
    /// `DoRockSlideSpecialEffects`: shake the screen horizontally and
    /// vertically at counters 8..=11; flash at counter 1.
    RockSlide,
    /// `DoGrowlSpecialEffects`: duplicate the first 4 OAM entries (second
    /// music note). Handled internally by the player; nothing emitted.
    Growl,
    /// `TailWhipAnimationUnused`: unreachable — Tail Whip's animation has
    /// no subanimations, so the hook never fires in the original either.
    TailWhipUnused,
    /// `DoBallTossSpecialEffects`: Poké Ball flow — depends on battle state
    /// the player does not have (wCurItem, wIsInBattle, Ghost Marowak).
    BallToss,
    /// `DoBallShakeSpecialEffects`: Poké Ball flow — restarts the
    /// subanimation wNumShakes times, which is capture logic, not playback.
    BallShake,
    /// `DoPoofSpecialEffects`: plays SFX_BALL_POOF at counter 5; sound
    /// playback is a frontend concern, nothing visual is emitted.
    BallPoof,
    /// `TradeHidePokemon`: trade sequence only, never used in battle.
    TradeHidePokemon,
    /// `TradeShakePokeball`: trade sequence only, never used in battle.
    TradeShakePokeball,
    /// `TradeJumpPokeball`: trade sequence only, never used in battle.
    TradeJumpPokeball,
}
...
// (constants/move_constants.asm); animation ids are 1-based.
