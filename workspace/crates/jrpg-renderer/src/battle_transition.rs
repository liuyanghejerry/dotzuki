...
// Tile $FF is the solid-black battle transition tile (loaded at vChars1 tile $7F in ASM)
...
// ── Circle arc data (engine/battle/battle_transitions.asm) ─────────
//
// BattleTransition_CircleData1-5: per-row (fill_count, skip_count) pairs,
// terminated by -1 ($FF). BattleTransition_Circle_Sub3 interprets one pair
// per tilemap row: fill N tiles toward the screen edge, step one row, then
// move the row start M tiles back toward the arc center.
...
// BattleTransition_HalfCircle1 — the top half of the circle wipe.
...
// BattleTransition_HalfCircle2 — the bottom half of the circle wipe.
...
/// Matches the ASM implementations in engine/battle/battle_transitions.asm
/// — writing tile $FF to specific tilemap positions at 60fps.
///
/// Copy-based transitions (Shrink/Split) additionally redirect each tile to
/// a shifted source tile (`src`), reproducing BattleTransition_CopyTiles1/2:
...
pub struct BattleTransitionState {
    ...
    /// pure-wipe transitions; Shrink/Split shift these as the ASM tile
    /// copies do. (Black tiles ignore this.)
    src: Vec<(u8, u8)>,
    width_tiles: usize,
    height_tiles: usize,
    done: bool,
...
impl BattleTransitionState {
    ...
    fn tick_spiral_inward(&mut self) {
        // ASM: BattleTransition_InwardSpiral (engine/battle/battle_transitions.asm)
        // Walks an inward spiral from the top-left corner, writing $FF (black)
        // at every tile until the screen is filled. The original ASM bursts
        // ~7 tiles per TransferDelay3 (~3 frames), giving roughly 2-3 tiles
        // per 60Hz frame; we match that by stepping a small batch per tick.
    ...
    fn tick_spiral_outward(&mut self) {
        // ASM: BattleTransition_OutwardSpiral (/ BattleTransition_Spiral)
        // Starts from center (10,10), fills 3 tiles per inner loop, 120 outer loops.
        // Direction rotates when hitting a filled tile: up → left → down → right
        ...
        // Write 3 tiles per frame (ASM: inner loop c=3)
        for _ in 0..3 {
            if self.spiral_x >= 0 && self.spiral_x < w && self.spiral_y >= 0 && self.spiral_y < h {
    ...
    /// Port of BattleTransition_Circle_Sub3: draw one `half_circle` arc
    /// entry. `quadrant_y` selects the row-step direction (0 = down for
    /// HalfCircle1, 1 = up for HalfCircle2).
    fn draw_arc(&mut self, entry: &HalfCircleEntry, quadrant_y: u8) {
        ...
        loop {
            ...
            // preserved (ASM `push hl` / `pop hl`) — the fill does NOT move
    ...
    fn tick_circle(&mut self) {
        // ASM: BattleTransition_Circle — plays half-circle1 (top) fully, then
        // half-circle2 (bottom). Circle_Sub1 draws ONE arc entry per call,
        // with TransferDelay3 (~3 frames) between entries.
        if self.counter > 0 {
            self.counter -= 1;
            return;
        }
    ...
    fn tick_double_circle(&mut self) {
        // ASM: BattleTransition_DoubleCircle — animates BOTH half circles at
        // the same time: one entry from each per TransferDelay3 (~3 frames).
        if self.counter > 0 {
            self.counter -= 1;
            return;
        }
    ...
    fn tick_horizontal_stripes(&mut self) {
        // ASM: BattleTransition_HorizontalStripes
        ...
        // ASM uses TransferDelay3 (~3 frames) between every column transfer.
        if self.counter > 0 {
            self.counter -= 1;
            return;
        }
        ...
        self.frame += 1;
        self.counter = 3; // TransferDelay3
    }

    // ── Vertical Stripes ────────────────────────────────────────────

    fn tick_vertical_stripes(&mut self) {
        // ASM: BattleTransition_VerticalStripes
    ...
    /// the ASM tilemap byte copy, which also carries already-black tiles.
    ...
    fn tick_shrink(&mut self) {
        // ASM: BattleTransition_Shrink (battle_transitions.asm:362-389)
        // 9 steps (SCREEN_HEIGHT/2), DelayFrames 6 between steps. Each step
        // tile-COPIES rows/columns one tile toward the screen center
        // (BattleTransition_CopyTiles1/2), so the overworld visibly
        // COMPRESSES; the freed outer row/column is filled with tile $FF.
        if self.counter > 0 {
            self.counter -= 1;
            return;
        }
        ...
        // Vertical (CopyTiles1): top half rows shift DOWN one row
        // (hl=(0,7) de=(0,8) off=-40 → rows 0..7 copied into 1..8),
        // bottom half rows shift UP one row
        // (hl=(0,10) de=(0,9) off=+40 → rows 10..17 copied into 9..16).
        for y in (1..rmid).rev() {
            for x in 0..w {
        ...
        // Horizontal (CopyTiles2): left half cols shift RIGHT one col
        // (hl=(8,0) de=(9,0) off=-2 → cols 0..8 copied into 1..9),
        // right half cols shift LEFT one col
        // (hl=(11,0) de=(10,0) off=+2 → cols 11..19 copied into 10..18).
        for x in (1..cmid).rev() {
            for y in 0..h {
        ...
        self.frame += 1;
        self.counter = 5; // DelayFrames 6
    }

    // ── Split ───────────────────────────────────────────────────────

    fn tick_split(&mut self) {
        // ASM: BattleTransition_Split (battle_transitions.asm:392-417)
        // 9 steps, TransferDelay3 + Delay3 (~6 frames) between steps. Each
        // step tile-COPIES rows/columns one tile AWAY from the center, so
        // the overworld visibly SPLITS apart; the freed center row/column
        // is filled with tile $FF.
        if self.counter > 0 {
            self.counter -= 1;
            return;
        }
        ...
        // Vertical (CopyTiles1): bottom half rows shift DOWN one row
        // (hl=(0,16) de=(0,17) off=-40 → rows 9..16 copied into 10..17),
        // top half rows shift UP one row
        // (hl=(0,1) de=(0,0) off=+40 → rows 1..8 copied into 0..7).
        ...
        // Horizontal (CopyTiles2): right half cols shift RIGHT one col
        // (hl=(18,0) de=(19,0) off=-2 → cols 10..18 copied into 11..19),
        // left half cols shift LEFT one col
        // (hl=(1,0) de=(0,0) off=+2 → cols 1..9 copied into 0..8).
        ...
        self.frame += 1;
        self.counter = 5; // TransferDelay3 + Delay3 = 6 frames
    }
...
mod tests {
    ...
    // ── Circle arc rendering (BattleTransition_Circle_Sub3) ─────────
    ...
    // ── Shrink / Split tile copies (BattleTransition_CopyTiles1/2) ──
    ...
    fn test_shrink_first_step_compresses_toward_center() {
        ...
        // then horizontally on the shifted content (ASM order: CopyTiles1
        // top/bottom, then CopyTiles2 left/right). So dest (5,1) shows the
    ...
    fn test_split_first_step_pushes_away_from_center() {
        ...
        // then horizontally on the shifted content (same ASM ordering as
    ...
    fn test_shrink_completes_fully_black() {
        // 9 steps × 6 frames (DelayFrames 6).
        let mut state = BattleTransitionState::new(BattleTransitionKind::Shrink, 20, 18);
        let mut ticks = 0;
        while !state.is_done() && ticks < 120 {
            state.tick();
