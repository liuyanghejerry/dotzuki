...
A `cargo-generate` template for creating JRPG games using the `jrpg-engine` workspace.
...
```
my-jrpg/
├── Cargo.toml         # Depends on jrpg-engine, jrpg-engine-tiled, jrpg-engine-script, jrpg-engine-dsl
├── src/
...
5. Rebuild — the DSL compiler in `jrpg-engine-dsl` compiles scenes to JavaScript at build time
...
| Crate | Purpose |
|-------|---------|
| `jrpg-engine` | Core types: Camera, Tilemap, MapLayer, MapRenderState, CollisionType |
| `jrpg-engine-tiled` | Tiled .tmx JSON parser → jrpg-engine types |
| `jrpg-engine-script` | JavaScript scripting engine (Boa) for map event scripts |
| `jrpg-engine-dsl` | DSL compiler — `.scene` / `.theme` → JavaScript + JSON |
