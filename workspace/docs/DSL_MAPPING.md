...
```
```js
await game.showText("PROF: Hello there!");   // emitted verbatim, no "Name: "
```
...
```
```js
await game.showText("Prof: Follow me!");   // identical output to @speaker
```
...
```
@if (hasStarter == true) {
    @speaker("Prof") { "You already have a monster." }
} @else {
    @choice {
        @option("Flambit") {
            @speaker("Prof") { "A fiery choice!" }
        }
        @option("Aquakit") {
            @speaker("Prof") { "A water type!" }
        }
    }
}
```
...
if (game.getFlag("HAS_STARTER")) {
    await game.showText("Prof: You already have a monster.");
} else {
    const choice = await game.showChoice(["Flambit", "Aquakit"]);
    if (choice === 0) {
        await game.showText("Prof: A fiery choice!");
    } else {
        await game.showText("Prof: A water type!");
    }
}
```
...
@trigger(
  ...
  script  = "PALLETTOWN_PROF",    //                       → npcs[].scriptId
...
- A storyline may carry **several** `@trigger` lines when multiple objects
  route to one handler (e.g. ProfLab's two DEX balls → `talkDex`); each
...
{
  "routes": [
    { "map": "ProfLab", "npc": "Prof", "storyline": "prof_ask", "after": null },
    { "map": "ViridianMart", "onEnter": true, "storyline": "mart_pickup", "after": "prof_ask" },
    { "map": "ProfLab", "npc": "Prof", "storyline": "prof_delivery", "after": "mart_pickup" }
  ]
}
```
...
```js
export async function storyline_prof_ask() { ... }
export async function storyline_mart_pickup() { ... }
export async function storyline_prof_delivery() { ... }
```
...
(`prof_parcel.scene`) moved to the pokered game repository with the repo split.
...
```
@storyline("start") {
  @trigger(map = "PalletTown", npc = "Prof")
  @run {
