...
如何制作 `.gui` 布局：20×18 瓦片网格、面板与文本、模板绑定、双语标签、自定义组件，以及预览-检查循环。
...
所有定位都是绝对定位，画布是 20×18 的瓦片网格。每个组件都带一个 `rect`：
...
`style` 选择边框（`"default"`、`"single"`、`"double"`，或瓦片 id 的自定义对象）；`wrap = "word"` 让文本在 `rect` 内换行。编译出的 JSON 是纯数据——渲染器绘制的形状：
...
内置组件覆盖面板、文本、瓦片、列表与光标。游戏专属的部件（血条、队伍卡片）在共享文件里声明、按名字使用；编译器对每个使用点做声明校验：
