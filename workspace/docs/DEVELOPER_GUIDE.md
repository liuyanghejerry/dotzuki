...
| 对话系统 | `TextProvider` | `pokered-core` 的文本 Provider 实现 |
...
let connections = MapConnections {
    ...
    south: Some(MapConnection::new(Direction::Down, StartTown, 0)), // 南边连到起始镇
    west: None,
    east: None,
};
```
...
fn species_name(species: SpeciesId, is_zh: bool) -> &'static str {
    if is_zh {
        match species {
            SpeciesId::Leafkit => "叶芽兽",
            SpeciesId::Sparkit => "电鼠兽",
            // ...
        }
    } else {
        match species {
            SpeciesId::Leafkit => "LEAFKIT",
            SpeciesId::Sparkit => "SPARKIT",
            // ...
        }
    }
}
...
```rust
// 每帧检查
let triggered: Vec<String> = trigger_mgr.check_triggers("StartTown", player_x, player_y);
...
// A 键交互
if input.a {
    if let Some(script_name) = trigger_mgr.check_interact_mut("StartTown", facing_x, facing_y) {
        script_engine.call_function(&script_name);
    }
}
```
...
脚本引擎通过 `coord_event_fn(x, y)` 或 `coord_event_by_name(name)` 查找对应的 JS 函数名。`.scene` 文件中的 `@trigger(name = "northExit1")` 参数直接映射到此 `name` 字段。建议为每个 coord event 赋予有意义的名称（如 `northExit1`、`profLabTrigger`），提高可读性。
...
```rust
use dotzuki_engine::palette::{PaletteTrait, PaletteProvider, SgbColor};
