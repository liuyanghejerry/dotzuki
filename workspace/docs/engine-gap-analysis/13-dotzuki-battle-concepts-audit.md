...
Strategy, FFX/X-2, SaGa) measured against what `dotzuki-engine`'s `battle::stack` engine can
...
`dotzuki-engine::battle::stack` is a **Showdown-style effect-stack** battle engine: native-Rust
