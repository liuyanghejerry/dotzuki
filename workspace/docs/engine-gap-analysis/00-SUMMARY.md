...
charter (`jrpg-engine` = reusable, pokered = one example), the engine today gives
...
> Verified: jrpg-engine 290 tests pass / 0 fail; pokered-core 1802 pass / 0 fail;
...
> **jrpg-engine 292 / pokered-core 1821**. See the report's top "Update" section.
...
- **P0 — ✅ engine layer landed; Pokemon-struct re-point staged** (`792c8410`) Generic party/monster model (EXP, level-up, stats, evolution) lives in `jrpg-engine/party/`; production leveling still flows through pokered's `process_level_up` (the engine's `gain_exp` is not yet a drop-in — see report §party).
...
| Robust save framework (dual-slot rotation, CRC, corruption recovery) | Engine save is single-blob CRC only | `jrpg-engine/save/` (`SaveManager`, CRC16) | `save.c`, `save_failed_screen.c` (14 sectors) | M | **P1** |
...
| Map authoring toolchain + docs (and XML Tiled support) | New devs need a documented map pipeline; parser is JSON-only | `jrpg-engine-tiled` (JSON `.tmx`), `tools/` | `mapjson`, `layouts.json`, porymap docs | S–M | **P2** |
| Cleanups: `game_api.rs` stub, `MenuSystem` `unsafe mem::zeroed()` | Correctness/clarity for engine consumers | `jrpg-engine-script/game_api.rs`, `menu/mod.rs:383` | n/a | S | **P2** |
...
generically-parameterized engine modules under `jrpg-engine/` (`battle/driver.rs`,
`battle/ai.rs`, `party/`, `overworld/encounter.rs`, `items/use_driver.rs`), each
with pokered provider impls + parity tests, landed and green (jrpg-engine 290/0,
...
- **Overworld** (`jrpg-engine/overworld/`): collision, grid movement, NPC
...
- **JS scripting bridge** (`jrpg-engine-script`): Boa async/await `ScriptCommand`
