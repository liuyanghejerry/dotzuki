...
1. **英文是源语言，中文翻译版必产**。面向用户（游戏作者、Rust 开发者）的
   每篇文档一律以英文撰写，并产出对应的中文翻译版，以 `-zh-CN` 后缀与
   英文源并列（如 `quickstart.md` → `quickstart.zh-CN.md`）。英文源是唯一
   权威：内容冲突时以英文为准，发现不一致先修英文源，再同步翻译。
2. **翻译版是派生品**。每篇翻译版在头部标注：

   ```markdown
   > 本文是 `xxx.md` 的中文翻译，同步至引擎版本 vX.Y.Z（源文档 commit <sha>）。
   > 内容以英文源为准；发现不一致请更新英文源再同步翻译。
   ```

3. **禁止单侧修改翻译版**。翻译版只能改措辞，不能增删技术内容。
4. **术语译名以 glossary 为准**。中文术语对照收录在 `reference/glossary.md`；
   翻译时不得自造译名，新增术语先补 glossary 对照再用于翻译。
