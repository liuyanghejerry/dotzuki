# dotzuki-editor — AI Agent Framework Design

Status: **in progress** — the spine + chat flagship have landed (see *Implementation status*) · Branch: `feature/editor-ai-integration`

Goal: deepen AI integration across *every* editing surface of dotzuki-editor by benchmarking against
...
| Generality | **Fully config-driven** | Zero wuxia/pokered hardcoding. Everything is driven off `.dotzuki-editor.json` activity config. |
...
> (The compile-check half is solved: `check_scene` now defaults to the bundled dotzuki-web
...
One module built from `.dotzuki-editor.json`, generalizing the `readStoryRecord` / `scanFlags` / `listSceneNames` /
...
- **M8 — Multi-session threads (P2, done) + background scheduled tasks (P3, done).** Threads shipped:
  ...
  review tray (never auto-applied); jobs persist per project in `.dotzuki-editor.jobs.json`.
...
- **Validators per surface:** GUI has `compile_gui`; scenes are covered by the bundled
  dotzuki-web WASM compiler (`check_scene` defaults to it, `scene.checkCmd` still wins)
