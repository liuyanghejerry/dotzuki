...
// exactly like today: it runs on this machine and on copies transferred without
// the download "quarantine" flag (USB, scp, git). Nothing below forces signing
// or notarization, so `pnpm electron:pack` / `electron:dist` keep working with
// zero setup.
//
...
module.exports = {
  ...
  mac: {
    ...
    // (ad-hoc) — today's behavior. Set CSC_NAME/CSC_LINK to sign.
