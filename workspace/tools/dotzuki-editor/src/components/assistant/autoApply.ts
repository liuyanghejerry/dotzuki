...
// / map-tilemap) are NEVER auto-applied: they reshape the project itself (the
// config file, a whole scaffold, new map dirs) or DESTROY content (a tilemap
// delete), so a human must always review them — shouldAutoApply hard-codes
// that guard and the UI offers no toggle for them.
// ───────────────────────────────────────────────────────────────────────────
...
/** Proposal target kinds that reshape the project or destroy content —
 *  manual review only, always. */
export const META_KINDS = ['project-config', 'project-scaffold', 'map-create', 'map-tilemap'] as const
