...
describe('auto-apply kind gates', () => {
  it('covers the five content kinds and the four meta kinds', () => {
    expect([...CONTENT_KINDS]).toEqual(['story', 'data', 'scene', 'gui', 'map'])
    expect([...META_KINDS]).toEqual(['project-config', 'project-scaffold', 'map-create', 'map-tilemap'])
  })
  ...
  it('NEVER auto-applies meta kinds, whatever the switches say', () => {
    const allOn = { story: true, data: true, scene: true, gui: true, map: true, 'project-config': true, 'project-scaffold': true, 'map-create': true, 'map-tilemap': true }
