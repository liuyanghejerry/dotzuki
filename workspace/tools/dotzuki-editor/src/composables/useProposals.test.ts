...
describe('useProposals.add — op threading', () => {
  it('carries a delete op through add and defaults missing ops to write', () => {
    const tray = useProposals()
    tray.add({ target: { kind: 'map-tilemap', map: 'Town', path: 'maps/Town/map.tmx.json' }, title: 'Delete tilemap of map "Town"', diff: [], before: '{"width":2}', after: '', op: 'delete' })
    tray.add({ target: { kind: 'scene', scene: 'Town', path: 'maps/Town/script.scene' }, title: 'Edit scene', diff: [], before: 'x', after: 'y' })
    expect(tray.proposals.value[0].op).toBe('delete')
    expect(tray.proposals.value[1].op).toBe('write') // absent → write (backwards compatible)
  })
})
