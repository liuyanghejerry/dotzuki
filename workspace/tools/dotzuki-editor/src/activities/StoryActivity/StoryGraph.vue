...
function statusClass(s: string): string {
  return {
    idea: 'border-border-strong',
    drafted: 'border-warning/70',
    scripted: 'border-accent-strong/70',
    done: 'border-green-500/70',
  }[s] ?? 'border-border-strong'
}
function statusDot(s: string): string {
  return {
    idea: 'bg-gray-500',
    drafted: 'bg-warning',
    scripted: 'bg-accent-strong',
...
<template>
  <div class="h-full overflow-auto p-4">
    <div class="flex items-center gap-4 mb-3 text-tiny text-ink-faint">
      <span class="font-semibold uppercase tracking-wide">{{ t('story.views.graph') }}</span>
      <span class="flex items-center gap-1"><i class="w-2 h-2 rounded-pill bg-gray-500 inline-block" /> {{ t('story.status.idea') }}</span>
      <span class="flex items-center gap-1"><i class="w-2 h-2 rounded-pill bg-warning inline-block" /> {{ t('story.status.drafted') }}</span>
      <span class="flex items-center gap-1"><i class="w-2 h-2 rounded-pill bg-accent-strong inline-block" /> {{ t('story.status.scripted') }}</span>
      <span class="flex items-center gap-1"><i class="w-2 h-2 rounded-pill bg-success-strong inline-block" /> {{ t('story.status.done') }}</span>
    ...
    <div v-if="!quests.length" class="h-64 flex items-center justify-center text-ink-disabled text-sm">
      {{ t('story.emptyGraph') }}
    </div>
    ...
    <div v-else class="relative" :style="{ width: canvasSize.w + 'px', height: canvasSize.h + 'px' }">
      ...
      <div
        ...
        :key="'h' + col"
        class="absolute text-tiny font-semibold text-ink-muted uppercase tracking-wide truncate"
      ...
      <button
        ...
        :key="n.q.id"
        @click="open(n.q.id)"
        class="absolute text-left bg-surface border rounded-md px-2.5 py-1.5 hover:bg-surface-hover transition-colors shadow-sm"
        :class="statusClass(n.q.status)"
        :style="{ left: n.pos.x + 'px', top: n.pos.y + 'px', width: NODE_W + 'px', height: NODE_H + 'px' }"
      >
        <div class="flex items-center gap-1.5">
          <span class="w-2 h-2 rounded-pill shrink-0" :class="statusDot(n.q.status)" />
          <span class="text-xs text-ink font-medium truncate">{{ questTitle(n.q) }}</span>
        </div>
        <div class="text-micro text-ink-faint truncate mt-0.5">
          {{ n.q.type }} · {{ n.q.id }}
        </div>
      </button>
    </div>
