...
export interface ChangeTarget {
  kind: 'story' | 'data' | 'scene' | 'gui' | 'map' | 'map-tilemap' | 'project-config' | 'project-scaffold' | 'map-create'
  ...
  /** gui: the layout name. */
  name?: string
  /** map / map-tilemap / map-create: the map directory name. */
  map?: string
...
export interface Proposal {
  ...
  /** Proposed content. */
  after: string
  /** How applying should mutate the target: 'write' replaces the file with
   *  `after` (default); 'delete' removes it (or, for `map-tilemap`, the whole
   *  tilemap artifact set). A delete's diff is `before` with everything removed. */
  op?: 'write' | 'delete'
  diff: DiffOp[]
}
