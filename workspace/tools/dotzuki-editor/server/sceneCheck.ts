...
//   2. Built-in WASM compile (default) — the nodejs-target dotzuki-web pkg
//      (`crates/dotzuki-web/pkg-node`, built by `pnpm build:wasm`) runs the real
...
async function loadWasmCompiler(): Promise<WasmSceneCompiler | null> {
  ...
  try {
    ...
    console.warn(
      `[sceneCheck] dotzuki-web nodejs WASM pkg not loadable (${(e as Error).message}); ` +
...
export async function checkScene(project: ProjectContext, sceneName: string, content: string): Promise<SceneCheckResult> {
  ...
  // Default: real compile via the dotzuki-web WASM compiler, then layer the lint
  ...
  if (await loadWasmCompiler()) {
    ...
    return {
      ok: !hasErr,
      source: 'compile',
      output: `compile PASS (dotzuki-web WASM compiler)\n${lintReport(findings)}`.slice(0, CAP),
    }
  }
