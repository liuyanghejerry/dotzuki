...
// (`.dotzuki-editor.jobs.json` in the project root, whole-array write-back) and
 ...
 * probe: it throws when no .dotzuki-editor.json exists, so the routes answer 500
 * exactly like the other project-scoped endpoints.
 */
export function jobsFile(): string {
  loadConfig()
  return path.join(getProjectRoot(), '.dotzuki-editor.jobs.json')
}
