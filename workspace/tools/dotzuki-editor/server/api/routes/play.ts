...
// dotzuki-runner (crates/dotzuki-runner-web) against an in-memory filesystem.
// ──────────────────────────────────────────────────────────────
 ...
 *   EXCEPT `.dotzuki-editor.json` (the runner needs the project manifest).
 *
  ...
  const walk = (dir: string, rel: string) => {
    for (const name of fs.readdirSync(dir).sort()) {
      ...
      if (st.isDirectory()) {
        ...
        if (name !== '.dotzuki-editor.json' && (name.startsWith('.') || name.endsWith('.bak'))) continue
        if (st.size > maxFile) {
