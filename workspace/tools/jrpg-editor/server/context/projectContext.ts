...
export class ProjectContext {
  ...
  /** Absolute path to a map's authored tilemap (`map.tmx.json`). Same traversal
   *  guard as mapObjectsPath. */
  mapTmxPath(name: string): string {
    const dir = this.mapsDir()
    if (!dir) throw new Error('project has no map activity / mapsDir configured')
    return path.join(dir, path.basename(String(name)), 'map.tmx.json')
  }
