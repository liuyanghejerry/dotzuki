...
//   map-tilemap      — delete/restore a map's whole authored tilemap artifact
//                      set (map.tmx.json + per-map tileset files); the backup
//                      carries all of them so Revert restores the full set.
...
import { createMap, createMapTmx } from '../api/mapCreate'
...
/** The file names that constitute a map's authored tilemap (dir-relative).
 *  Deleting a tilemap removes exactly this set; the map dir itself (map.json,
 *  objects.json, script.scene, source.png) is untouched. */
const TILEMAP_FILES = ['map.tmx.json', 'tileset.png', 'tileset.tiles.json'] as const

/**
 * Structured backup payload carried in the result `backup` string of a
 * tilemap delete — the single-string field can only hold ONE file, so the
 * whole set rides here and Revert (a write with this payload in `after`)
 * restores every file, PNG included.
 */
interface TilemapBackup {
  /** map.tmx.json content. */
  tmx: string | null
  /** tileset.png, base64. */
  tilesetPng: string | null
  /** tileset.tiles.json content. */
  tiles: string | null
}
...
function resolveTargetPath(project: ProjectContext, t: ChangeTarget): string {
  switch (t.kind) {
    ...
    case 'map-tilemap':
      if (!t.map) throw new Error('map-tilemap target needs map')
      return project.mapTmxPath(t.map)
    case 'project-config':
...
export function applyChange(project: ProjectContext, req: ApplyChangeRequest): ApplyChangeResult {
  ...
  if (req.op === 'delete') {
    ...
    if (req.target.kind === 'map-tilemap') {
      // Delete the map's authored tilemap (tmx + per-map tileset), keeping the
      // map itself. Backs up the whole set so Revert restores every file.
      return deleteMapTilemap(project, req.target, req.expect, req.force)
    }
  ...
  if (req.target.kind === 'map-tilemap') return restoreMapTilemap(project, req.target, req.after)
...
function applyMapCreate(project: ProjectContext, abs: string, req: ApplyChangeRequest): ApplyChangeResult {
  ...
  // Optional blank tilemap: the proposal's payload may carry width/height.
  try {
    const payload = req.after ? JSON.parse(req.after) : null
    if (payload && (Number(payload.width) > 0 || Number(payload.height) > 0)) {
      createMapTmx(project, { name: path.basename(abs), width: Number(payload.width), height: Number(payload.height) })
    }
  } catch (e) {
    throw new Error('map-create tilemap failed: ' + (e as Error).message)
  }
  return { ok: true, backup: null, path: path.relative(project.root, abs) }
}

/**
 * Delete a map's authored tilemap artifact set (TILEMAP_FILES), keeping the map
 * directory itself. The tmx content rides in `backup` alongside the tileset
 * files so a later write-with-backup restores the whole set. `expect`, when
 * provided, is the tmx content the proposal's diff was built against — a drift
 * (someone edited the tilemap since) refuses the delete, same as the write path.
 */
function deleteMapTilemap(project: ProjectContext, t: ChangeTarget, expect: string | null | undefined, force: boolean | undefined): ApplyChangeResult {
  const tmxAbs = project.mapTmxPath(t.map ?? '')
  const dir = path.dirname(tmxAbs)
  if (!fs.existsSync(tmxAbs) || !fs.statSync(tmxAbs).isFile()) {
    throw new Error(`map "${t.map}" has no tilemap (map.tmx.json) to delete`)
  }
  const readText = (f: string) => (fs.existsSync(f) && fs.statSync(f).isFile() ? fs.readFileSync(f, 'utf-8') : null)
  const readBin = (f: string) => (fs.existsSync(f) && fs.statSync(f).isFile() ? fs.readFileSync(f) : null)

  const curTmx = readText(tmxAbs)
  if (!force && expect !== undefined && curTmx !== expect) {
    return { ok: false, conflict: true, backup: curTmx, path: path.relative(project.root, tmxAbs) }
  }
  const pngRaw = readBin(path.join(dir, 'tileset.png'))
  const backup: TilemapBackup = {
    tmx: curTmx,
    tilesetPng: pngRaw ? pngRaw.toString('base64') : null,
    tiles: readText(path.join(dir, 'tileset.tiles.json')),
  }
  for (const f of TILEMAP_FILES) {
    const abs = path.join(dir, f)
    if (fs.existsSync(abs) && fs.statSync(abs).isFile()) fs.unlinkSync(abs)
  }
  return { ok: true, backup: JSON.stringify(backup), path: path.relative(project.root, tmxAbs) }
}

/**
 * Restore a deleted tilemap set from the structured backup the delete returned
 * (`after` carries the TilemapBackup JSON). This is the Revert path: no drift
 * guard, and it refuses only when the map directory itself is gone.
 */
function restoreMapTilemap(project: ProjectContext, t: ChangeTarget, after: string): ApplyChangeResult {
  const tmxAbs = project.mapTmxPath(t.map ?? '')
  const dir = path.dirname(tmxAbs)
  if (!fs.existsSync(dir) || !fs.statSync(dir).isDirectory()) {
    throw new Error(`map "${t.map}" no longer exists — cannot restore its tilemap`)
  }
  let b: TilemapBackup
  try { b = JSON.parse(after) } catch { throw new Error('tilemap restore needs the structured backup payload in after') }
  if (b?.tmx != null) fs.writeFileSync(path.join(dir, 'map.tmx.json'), b.tmx, 'utf-8')
  if (b?.tilesetPng) fs.writeFileSync(path.join(dir, 'tileset.png'), Buffer.from(b.tilesetPng, 'base64'))
  if (b?.tiles != null) fs.writeFileSync(path.join(dir, 'tileset.tiles.json'), b.tiles, 'utf-8')
  return { ok: true, backup: null, path: path.relative(project.root, tmxAbs) }
}
