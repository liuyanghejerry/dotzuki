...
function assistantSystemPrompt(project: ProjectContext, mentions: MentionTarget[], uiContext?: UiContext, memories: Memories = NO_MEMORIES, skills: SkillMeta[] = [], allowCodeExecution = false): string {
  ...
  return [
    ...
    '- To place NPCs, warps, or collision on a map, edit its objects.json via propose_map_edit (`map` = the map directory name from list_maps; `content` = the COMPLETE objects.json). To change the project config, use propose_project_config with the COMPLETE new .jrpg-editor.json (read it first with read_file).',
