...
/**
 * The editor repo ships a package.json named "jrpg-editor" — that is how we
 * detect "the dev server was started from the editor's own repo" (the usual
 * `pnpm dev` from tools/jrpg-editor case).
 */
function looksLikeEditorRepo(cwd: string): boolean {
  try {
    const pkg = JSON.parse(fs.readFileSync(path.join(cwd, 'package.json'), 'utf-8'))
    return pkg?.name === 'jrpg-editor'
  } catch {
    return false
  }
}
 ...
 * - A cwd holding a `.jrpg-editor.json` is a game project — keep it.
...
export function defaultProjectRoot(env: NodeJS.ProcessEnv, cwd: string, homedir: string): string {
  ...
  if (fs.existsSync(path.join(cwd, '.jrpg-editor.json'))) return cwd
