...
describe('GET /api/play/bundle', () => {
  it('answers 400 when no project is open (no .jrpg-editor.json)', async () => {
  ...
  it('returns base64 file map with posix paths + projectRoot', async () => {
    ...
    expect(Object.keys(body.files).sort()).toEqual(['.jrpg-editor.json', 'data/maps/hello.json'])
    expect(Buffer.from(body.files['data/maps/hello.json'], 'base64').toString('utf-8')).toBe('{"a":1}')
  })

  it('skips node_modules/.git/target/dist, *.bak and dotfiles — but keeps .jrpg-editor.json', async () => {
    writeProjectConfig(root())
    const files = [
    ...
    const res = await call(server.routes, '/api/play/bundle', mockReq('GET'))
    expect(res.status).toBe(200)
    expect(Object.keys(res.json().files).sort()).toEqual(['.jrpg-editor.json', 'data/keep.json'])
  })
  ...
  it('never follows symlinks, even ones escaping the project root', async () => {
    ...
    try {
      ...
      const res = await call(server.routes, '/api/play/bundle', mockReq('GET'))
      expect(res.status).toBe(200)
      expect(Object.keys(res.json().files)).toEqual(['.jrpg-editor.json'])
    } finally {
...
describe('collectProjectFiles (size caps via options)', () => {
  ...
  it('uses posix separators in keys on every platform', () => {
    ...
    expect(Object.keys(files).sort()).toEqual(['.jrpg-editor.json', 'deep/deeper/f.txt'])
  })
})

// ───────────────────────────────────────────────────────────────────────────
// /wasm fallback — the playtest runner pkg (jrpg-runner-web) is served by the
// /wasm middleware in content.ts when a file isn't in jrpg-web/pkg. Env roots
...
describe('GET /wasm/* runner-pkg fallback', () => {
  ...
  it('serves files missing from jrpg-web/pkg out of the runner pkg', async () => {
    ...
    try {
      fs.writeFileSync(path.join(webDir, 'jrpg_web.js'), '// web')
      fs.writeFileSync(path.join(runnerDir, 'jrpg_runner_web.js'), '// runner')
      fs.writeFileSync(path.join(runnerDir, 'jrpg_runner_web_bg.wasm'), 'wasm-bytes')

      // Present only in the runner pkg → fallback serves it.
      const a = await callWasm('/wasm/jrpg_runner_web.js')
      ...
      const b = await callWasm('/wasm/jrpg_runner_web_bg.wasm')
      ...
      const c = await callWasm('/wasm/jrpg_web.js')
