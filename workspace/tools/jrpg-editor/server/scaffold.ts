...
// system: just .jrpg-editor.json + data/gfx folders and starter content (demo
...
// Scenes compile to JavaScript and run on the jrpg-engine runtime.
//
// Where to look next:
...
export interface ScaffoldResult {
  /** The config written to .jrpg-editor.json. */
  config: {
    name: string
    dataRoot: string
    gfxRoot: string
...
export function scaffoldProject(targetDir: string, opts: ScaffoldOptions): ScaffoldResult {
  ...
  write('.jrpg-editor.json', JSON.stringify(config, null, 2))
