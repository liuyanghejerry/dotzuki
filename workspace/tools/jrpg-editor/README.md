...
A **game-agnostic** visual editor for games built on [jrpg-engine](../../crates/jrpg-engine/).  
Configure it once via `.jrpg-editor.json`, then edit maps, scripts, data tables, and assets — without writing code.

## Quick Start

A **game project** is a zero-Rust directory: a `.jrpg-editor.json` manifest
...
`.jrpg-editor.providers.json`, or globally in `~/.jrpg-editor/providers.json`
...
Create a `.jrpg-editor.json` in your game project root. This is the same
...
```bash
npx vite --config ../path/to/jrpg-editor/vite.config.ts
```

Or run `pnpm dev` from `tools/jrpg-editor/` itself. The dev server listens on **http://localhost:5174**.

### From the command line: `jrpg` CLI

The [`jrpg` CLI](../../crates/jrpg-cli/) scaffolds, checks, and *plays* game
...
via `jrpg-engine-dsl`'s runtime `compile_dirs` API. `run` (via the
`jrpg-runner` crate) boots the entry map — overworld walking, NPC dialogue
and choices, warps, music/SFX from `data/audio/**/*.json`, save/load at
`<project>/.jrpg-save.json` — with the exact behavior specified in
...
> is served from `crates/jrpg-web/pkg`. `pnpm electron:build` stages that pkg
...
> (`crates/jrpg-runner-web/pkg`, built by `pnpm build:wasm-runner`, staged to
> `dist-electron/wasm-runner-pkg`, located via `JRPG_RUNNER_WASM_ROOT`). The
> `/wasm` middleware serves `jrpg-runner-web/pkg` as a fallback for filenames
> not found in `jrpg-web/pkg`.

## How It Works

1. The editor reads `.jrpg-editor.json` from the current working directory
2. It exposes a Vite dev server with API endpoints for reading/writing game data
3. The UI renders activity tabs based on your config — no Pokémon hardcoding
...
└── src/
    ...
    ├── router.ts
    ├── types/
    │   └── project.ts       # .jrpg-editor.json TypeScript types
    ├── stores/
    ...
    │   ├── PlayActivity/    # In-editor playtest (WASM jrpg-runner canvas)
...
| **Play** | `play` | In-editor playtest — runs the project in the WASM jrpg-runner (see below) |

## Play (in-editor playtest)

The `play` activity runs the project **inside the editor** — no Rust toolchain,
no external terminal. It boots the WASM build of `jrpg-runner`
(`crates/jrpg-runner-web`, the same `RunnerGame` that powers `jrpg run`) on a
canvas: walk, talk to NPCs, battle, shop, save.

- **Build the bundle first**: `pnpm build:wasm-runner` (wasm-pack →
  `crates/jrpg-runner-web/pkg`). Without it the activity shows a setup hint.
...
For existing projects, add it to `.jrpg-editor.json` manually:
...
stored **without API keys** — per project in `.jrpg-editor.providers.json`,
falling back to a global `~/.jrpg-editor/providers.json` when no project is
...
2. **Built-in WASM compile (default)** — the editor loads the real Game DSL
   compiler in-process (nodejs-target jrpg-web pkg, `crates/jrpg-web/pkg-node`,
...
`.jrpg-editor.jobs.json`; a badge on the 🕒 button counts unreviewed runs.
...
`.jrpg-editor.image-providers.json`), `kind`:
...
3. Create `.jrpg-editor.json` in your project root
4. Define your data tables with field schemas
5. Run the editor from your project root

The editor never requires you to modify its source code. Everything is driven by your `.jrpg-editor.json` configuration.
