...
<template>
  <div class="border border-gray-700 rounded bg-gray-850/60">
    ...
    <div v-if="selectable" class="px-2.5 pb-1 text-[10px] text-gray-500">{{ t('assistant.selectHunksHint') }}</div>
    ...
    <div v-if="scaffold" class="mx-2.5 mb-2 rounded bg-gray-900 px-2.5 py-2 space-y-1 text-[11px]">
      <div class="flex gap-2"><span class="text-gray-500 w-16 shrink-0">{{ t('wizard.summaryName') }}</span><span class="text-gray-100">{{ scaffold.name }}</span></div>
      <div class="flex gap-2"><span class="text-gray-500 w-16 shrink-0">{{ t('wizard.summaryDir') }}</span><span class="text-gray-300 font-mono text-[10px] break-all">{{ scaffold.dir }}</span></div>
      <div class="flex gap-2"><span class="text-gray-500 w-16 shrink-0">{{ t('wizard.summaryTemplate') }}</span><span class="text-gray-300">{{ t(`templates.${scaffold.templateId}.name`) }}</span></div>
      <div v-if="scaffoldActivities" class="flex gap-2"><span class="text-gray-500 w-16 shrink-0">{{ t('assistant.scaffoldActivities') }}</span><span class="text-gray-400">{{ scaffoldActivities }}</span></div>
    </div>
    ...
    <div class="flex items-center justify-end gap-1.5 px-2.5 pb-2">
      ...
      <template v-if="proposal.status === 'pending' || proposal.status === 'failed'">
        <button @click="emit('discard')" class="px-2 py-0.5 text-[11px] rounded text-gray-400 hover:text-gray-200">{{ t('assistant.discard') }}</button>
      ...
      <template v-else-if="proposal.status === 'conflict'">
        <button @click="emit('discard')" class="px-2 py-0.5 text-[11px] rounded text-gray-400 hover:text-gray-200">{{ t('assistant.discard') }}</button>
        <button @click="emit('forceApply')" class="px-2.5 py-0.5 text-[11px] rounded bg-amber-700 text-white hover:bg-amber-600">{{ t('assistant.applyAnyway') }}</button>
      </template>
      <button v-else-if="proposal.status === 'applied'" @click="emit('revert')" class="px-2 py-0.5 text-[11px] rounded text-amber-400 hover:text-amber-300">{{ t('assistant.revert') }}</button>
    </div>
  </div>
</template>
