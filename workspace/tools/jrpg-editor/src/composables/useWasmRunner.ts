// ───────────────────────────────────────────────────────────────────────────
// The jrpg-runner WASM playtest bridge (crates/jrpg-runner-web, built by
...
// middleware (which falls back to crates/jrpg-runner-web/pkg when a file isn't
// in jrpg-web/pkg). The module is a module-level singleton cache.
//
// WASM contract (see crates/jrpg-runner-web):
...
export function loadRunnerModule(): Promise<WasmRunnerModule> {
  ...
  initPromise = (async () => {
    try {
      const wasmJsUrl = new URL('/wasm/jrpg_runner_web.js', window.location.origin).href
      ...
      throw new Error(
        `Failed to load the runner WASM: ${(e as Error).message} — ` +
          `please run \`pnpm build:wasm-runner\` first (crates/jrpg-runner-web/pkg).`,
      )
    }
  })()
  return initPromise
