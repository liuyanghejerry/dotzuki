...
// The generic jrpg-web WASM bridge. `render_gui` is the game-agnostic entry:
// compiles `.gui` source, injects a theme, binds editor data, and rasterises at
// an arbitrary size (see crates/jrpg-web/src/lib.rs).
  ...
  async function ensureInit(): Promise<void> {
    ...
    initPromise = (async () => {
      try {
        ...
        const wasmJsUrl = new URL('/wasm/jrpg_web.js', window.location.origin).href
