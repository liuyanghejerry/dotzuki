...
// (`jrpg-web`'s `render_audio_pcm`, backed by the jrpg-audio APU/sequencer) to
...
function ensureWasm(): Promise<AudioWasm> {
  ...
  if (!initPromise) {
    initPromise = (async () => {
      ...
      const url = new URL('/wasm/jrpg_web.js', window.location.origin).href
...
export function useAudioPlayback() {
  ...
  async function play(track: unknown, maxSeconds = 10): Promise<void> {
    ...
    try {
      ...
      const rate = w.audio_sample_rate()
  ...
  return { playing, rendering, error, play, stop }
}
