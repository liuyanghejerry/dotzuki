...
// Same story for the playtest runner pkg (crates/jrpg-runner-web), shipped as
...
// And the nodejs-target jrpg-web pkg (crates/jrpg-web/pkg-node), shipped as
...
async function openProjectDialog() {
  ...
  const result = await dialog.showOpenDialog(target ?? undefined, {
    ...
    message: 'Choose a folder containing .jrpg-editor.json',
  ...
  try {
    ...
    if (!resp.ok) {
      // 404 = no .jrpg-editor.json — offer to scaffold a fresh project in place.
      if (resp.status === 404) {
