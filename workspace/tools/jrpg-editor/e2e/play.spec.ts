...
// Play activity: boots the WASM jrpg-runner against the playable-game fixture
...
// crates/jrpg-runner-web/pkg). The spec skips itself when it isn't built.
...
test.describe('play activity (wasm runner)', () => {
  test('boots the game, responds to input and persists a save', async ({ page, request }) => {
    test.skip(
      (await request.get(`${playBase}/wasm/jrpg_runner_web.js`)).status() === 404,
